...
$sql = "select * from food join ingredient_food on ingredient_food.food_id = food.id join (select name as ingredient_name, id as ing_id from ingredient) as ingredient on ingredient.ing_id = ingredient_food.ingredient_id where food.id =" . $_POST['id'] .";";
...
if ($resultCheck > 0) {
    while ($row = mysqli_fetch_assoc($results)) {
        ...
        $foodBasicPrice = $row['basic_price'];
        $foodPrepTime = $row['prep_time'];
        $foodPhoto = $row['photo'];
        $rating = $row['rating'];
    }
}
...
<div class="w-full h-full add-kitchen-form hidden overflow-y-auto flex-col">
    ...
    <div class="flex mt-10 items-start">
        ...
        <div class="card-kitchen mb-4 w-72 overflow-hidden relative flex flex-col cursor-pointer border border-gray-300 rounded-2xl p-5 ml-5 transform transition duration-200 hover:bg-white hover:border-opacity-0 hover:shadow-2xl hover:scale-105">
            ...
            <div class="flex items-center flex-1 mb-2">
                <div class="w-16 h-16 rounded-full overflow-hidden flex-1">
                    <img class="object-contain w-full h-full rounded-full" <?php if($foodPhoto != '') {  echo "src='./photo_uploads/foods/$foodPhoto'"; } else { echo "src='./assets/featured/featured-burger.png'"; } ?> id="card-food-image" alt="foodImage">
                </div>
                <div>
            ...
            <p class="text-xs text-gray-400 my-1 card-description"><?php echo $foodDescription; ?></p>
            <h1 class="text-gray-600 font-semibold flex-1 card-foodName"><?php echo $foodName; ?></h1>
            ...
            <div class="flex items-center mt-2 justify-between">
                ...
                <div class="text-gray-500 text-xs flex flex-col items-end">
                    <h1 class="text-gray-600 font-semibold flex-1 card-foodName text-xl card-basicPrice"><?php echo "Rs." . $foodBasicPrice; ?></h1>
                    Disappearing Status
                </div>
            </div>
        ...
        <div class="flex flex-1 px-8">
            ...
            <div class="flex flex-col flex-1 px-6">
                <input type="text" placeholder="Food Name" class="mb-5 rounded-md bg-transparent" id="foodName" name="name" value="<?php echo $foodName; ?>">

                <textarea name="description" id="foodDescription" class="appearance-none mb-5 py-2 px-3 text-gray-700 leading-tight focus:outline-none bg-transparent focus:shadow-outline rounded-md transform transition-colors duration-300" placeholder="Description"><?php echo $foodDescription; ?></textarea>

                <input type="number" placeholder="Basic Price" class="mb-5 rounded-md transform transition-colors duration-300 bg-transparent" id="unitPriceFood" name="unitprice" value="<?php echo $foodBasicPrice; ?>">
                ...
                <div class="flex items-center">
                    <div>
                        ...
                        <input type="number" placeholder="Time" class="mb-5 bg-transparent rounded-md transform transition-colors duration-300" id="foodPrepTime" name="preparationtime" value="<?php echo $foodPrepTime; ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="w-80 h-64 rounded-lg overflow-hidden relative cursor-pointer profile-picture p-1 border-2 foodImageContainer shadow-2xl">
                ...
                <img id="foodUploadedPhoto" class="rounded-lg opacity-80 w-full h-full object-cover" <?php if($foodPhoto != '') {  echo "src='./photo_uploads/foods/$foodPhoto'"; } else { echo "src='./assets/featured/featured-burger.png'"; } ?> alt="Food Photo">
    ...
    <div class="flex-col my-5 px-12 selectedTextFood">
