...
if (isset($_POST['id'])) {
    ...
    if (empty($foodId) || empty($foodname) || empty($basicprice) || empty($ingredientIds) || empty($ingredientQuantities)) {
        ...
        if (isset($_FILES['image'])) {
            ...
            $sql = "UPDATE food SET name=?, basic_price=?, prep_time=?, description=? WHERE id=?;";
            ...
            if (!mysqli_stmt_prepare($statement, $sql)) {
                ...
                $rating = 0.0;
                $bindFailed = mysqli_stmt_bind_param($statement, 'siisi', $foodname, $basicprice, $preptime, $fooddescription, $foodId);
                if ($bindFailed === false) {
                    echo htmlspecialchars($statement->error);
                    exit();
                }
