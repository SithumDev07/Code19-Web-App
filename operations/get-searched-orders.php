...
?>
    ...
    if (is_numeric($keyWord)) {
        $sql = "SELECT * FROM customer_order WHERE id=" . $keyWord . ";";
        $message = "No Results Found For ID-" . $keyWord;
    } else if (substr($keyWord, 0, 9) == "Customer:") {
        $customer = substr($keyWord, 9);
        $customerIDs = array();
        ...
        $sql = "SELECT * FROM customer_order WHERE customer_id IN ('" . $customerIDs . "');";
        $message = "No Results Found For Customer Name - " . $customer;
    } else if ($keyWord == 'takeaway' || $keyWord == 'delivery') {

        $sql = "SELECT * FROM customer_order WHERE delivery_method='" . $keyWord . "';";
        $message = "No Results Found For Delivery Method - " . $keyWord;
    } else {
        $sql = "SELECT * FROM customer_order WHERE status LIKE '%" . $keyWord . "%';";
        $message = "No Results Found For Order Status - " . $keyWord;
    }
...
function renderFinalData($allData)
{
